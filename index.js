...
function profilListesi(newArr){
  let arr = [];
  for(let i = 0; i < newArr.length; i++){
    arr[i] = newArr[i].profile;
  }
  return arr;
}
...
function fenomenEkle(fenomenler, numara, profilIsmi, takipciSayisi, gonderi, platform){
  const newFenomen = {
    "number": numara,
    "profile": profilIsmi,
...
function fenomenGonderimSayisi(fenomenler, profileName){
  for(let i = 0; i < fenomenler.length; i++){
    if(fenomenler[i].profile == profileName){
